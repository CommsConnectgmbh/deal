    ...
    <div style={{ minHeight:'100dvh', background:'#060606', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'24px', maxWidth:430, margin:'0 auto' }}>
      ...
      <div style={{ width:'100%', background:'#111', borderRadius:16, border:'1px solid rgba(255,184,0,0.1)', padding:24 }}>
        ...
        <div style={{ marginBottom:16 }}>
          ...
          <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={t('auth.emailPlaceholder')} style={inputStyle} onKeyDown={e => e.key === 'Enter' && handle()} />
        </div>
        <div style={{ marginBottom:24 }}>
          <label style={labelStyle}>{t('auth.password').toUpperCase()}</label>
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder={t('auth.passwordPlaceholder')} style={inputStyle} onKeyDown={e => e.key === 'Enter' && handle()} />
        </div>
        ...
        <div style={{ textAlign:'center', marginTop:16 }}>
          <Link href="/auth/forgot-password" style={{ color:'rgba(240,236,228,0.4)', fontSize:13 }}>
            {t('auth.forgotPassword')}
          </Link>
        </div>
        <div style={{ textAlign:'center', marginTop:12, fontSize:14, color:'rgba(240,236,228,0.5)' }}>
          {t('auth.noAccount')}{' '}
          <Link href="/auth/register" style={{ color:'#FFB800' }}>{t('auth.register')}</Link>
        </div>
      </div>
    </div>
  )
