...
export default function RivalsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data: rivData } = await supabase
      .from('rivalries')
      .select('*, rival:rival_id(id, username, display_name, level)')
      ...
      if (friendsFollowing?.length) {
        ...
        if (topIds.length > 0) {
          const { data: sugProfiles } = await supabase
            .from('profiles')
            .select('id, username, display_name, level, wins, deals_total')
            .in('id', topIds)
          if (sugProfiles) {
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#060606', paddingTop: 60, paddingBottom: 100 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 20px 20px' }}>
        <h1 className="font-display" style={{ fontSize: 26, color: '#f0ece4' }}>{t('rivals.title')}</h1>
        <button
          onClick={() => setAddOpen(true)}
          style={{ padding: '10px 16px', borderRadius: 10, border: '1px solid rgba(255,184,0,0.3)', background: 'transparent', color: '#FFB800', fontFamily: 'Cinzel, serif', fontSize: 9, letterSpacing: 1, cursor: 'pointer' }}
        >
      ...
      {/* Tab switcher */}
      <div style={{ display: 'flex', margin: '0 16px 20px', background: '#111', borderRadius: 10, padding: 4, gap: 2 }}>
        {(['rivals', 'suggested'] as const).map(t2 => (
          <button
            ...
            style={{
              ...
              background: tab === t2 ? 'rgba(255,184,0,0.10)' : 'transparent',
              color: tab === t2 ? '#FFB800' : 'rgba(240,236,228,0.35)',
              fontFamily: 'Cinzel, serif', fontSize: 9, letterSpacing: 0.5, cursor: 'pointer',
            }}
          >
      ...
      <div style={{ padding: '0 16px 100px' }}>
        ...
        {tab === 'rivals' && (
          loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '60px 0' }}>
              ...
              <p className="font-display" style={{ fontSize: 16, color: 'rgba(240,236,228,0.4)', marginBottom: 8 }}>{t('rivals.noRivals')}</p>
              <p style={{ fontSize: 13, color: 'rgba(240,236,228,0.3)', lineHeight: 1.6, fontFamily: 'Crimson Text, serif', maxWidth: 260, margin: '0 auto 20px' }}>
                {t('rivals.noRivalsText')}
              </p>
              <button onClick={() => setTab('suggested')} style={{ padding: '10px 24px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #CC8800, #FFB800)', color: '#000', fontFamily: 'Cinzel, serif', fontSize: 10, fontWeight: 700, letterSpacing: 1, cursor: 'pointer' }}>
                VORSCHLÄGE SEHEN
              </button>
            </div>
          ) : rivalries.map((r: any) => {
            ...
            return (
              <div key={r.id} style={{ background: '#111', borderRadius: 14, border: `1px solid ${isHeated ? 'rgba(255,184,0,0.25)' : 'rgba(255,255,255,0.05)'}`, padding: '16px', marginBottom: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                  <div
                    onClick={() => router.push(`/app/profile/${r.rival?.username}`)}
                    style={{ width: 48, height: 48, borderRadius: '50%', background: 'linear-gradient(135deg, #CC8800, #FFB800)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, cursor: 'pointer' }}
                  >
                    <span className="font-display" style={{ fontSize: 16, color: '#000', fontWeight: 700 }}>{initials(r.rival)}</span>
                  </div>
                  <div style={{ flex: 1, cursor: 'pointer' }} onClick={() => router.push(`/app/profile/${r.rival?.username}`)}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span className="font-display" style={{ fontSize: 13, color: '#f0ece4' }}>@{r.rival?.username}</span>
                      {isHeated && <span style={{ fontSize: 11 }}>🔥</span>}
                    </div>
                    <span style={{ fontSize: 12, color: 'rgba(240,236,228,0.4)' }}>Level {r.rival?.level || 1}</span>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <span className="font-display" style={{ fontSize: 14, color: '#4ade80' }}>{r.wins || 0}W</span>
                    <span style={{ fontSize: 13, color: 'rgba(240,236,228,0.3)', margin: '0 4px' }}>/</span>
                    <span className="font-display" style={{ fontSize: 14, color: '#f87171' }}>{r.losses || 0}L</span>
                  </div>
                </div>
                {/* Intensity bar */}
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    <span className="font-display" style={{ fontSize: 8, letterSpacing: 1, color: 'rgba(240,236,228,0.3)' }}>INTENSITÄT</span>
        ...
        {tab === 'suggested' && (
          loading ? (
            ...
            <>
              <p style={{ fontSize: 13, color: 'rgba(240,236,228,0.35)', fontFamily: 'Crimson Text, serif', marginBottom: 16 }}>
                {lang === 'de' ? 'Basierend auf deinem Netzwerk & aktiven Spielern' : 'Based on your network & active players'}
              </p>
              {suggested.map((u: any) => (
                <div key={u.id} style={{ background: '#111', borderRadius: 14, border: '1px solid rgba(255,255,255,0.05)', padding: '14px 16px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div
                    onClick={() => router.push(`/app/profile/${u.username}`)}
                    style={{ width: 46, height: 46, borderRadius: '50%', background: 'linear-gradient(135deg, #CC8800, #FFB800)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, cursor: 'pointer' }}
                  >
                    <span className="font-display" style={{ fontSize: 14, color: '#000', fontWeight: 700 }}>{initials(u)}</span>
                  </div>
                  <div style={{ flex: 1, minWidth: 0, cursor: 'pointer' }} onClick={() => router.push(`/app/profile/${u.username}`)}>
                    <p style={{ fontSize: 13, color: '#f0ece4', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {u.display_name || u.username}
                    </p>
                    <p style={{ fontSize: 11, color: 'rgba(240,236,228,0.4)' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background: isAlreadyRival(u.id) ? 'transparent' : 'linear-gradient(135deg, #CC8800, #FFB800)',
                      color: isAlreadyRival(u.id) ? '#4ade80' : '#000',
                      fontFamily: 'Cinzel, serif', fontSize: 9, fontWeight: 700, letterSpacing: 1, flexShrink: 0,
                    }}
                  >
      ...
      {addOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'flex-end', zIndex: 200 }} onClick={() => setAddOpen(false)}>
          <div style={{ width: '100%', maxWidth: 430, margin: '0 auto', background: '#111', borderRadius: '20px 20px 0 0', border: '1px solid rgba(255,184,0,0.15)', padding: '24px 20px 48px' }} onClick={e => e.stopPropagation()}>
            ...
            <h3 className="font-display" style={{ fontSize: 16, color: '#FFB800', textAlign: 'center', marginBottom: 16 }}>RIVAL HINZUFÜGEN</h3>
            <input
              value={search}
              onChange={e => { setSearch(e.target.value); searchUsers(e.target.value) }}
              placeholder="Username suchen…"
              style={{ width: '100%', padding: '13px 16px', background: '#1a1a1a', border: '1px solid rgba(255,184,0,0.15)', borderRadius: 10, color: '#f0ece4', fontSize: 15, outline: 'none', marginBottom: 12, boxSizing: 'border-box' }}
              autoFocus
            />
            {searchRes.map((u: any) => (
              <div key={u.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <div>
                  <p style={{ color: '#f0ece4', fontSize: 14 }}>{u.display_name || u.username}</p>
                  <p style={{ color: 'rgba(240,236,228,0.4)', fontSize: 12 }}>@{u.username} · Lv.{u.level}</p>
                </div>
                <button
                  onClick={() => addRival(u.id)}
                  disabled={isAlreadyRival(u.id)}
                  style={{ padding: '8px 14px', borderRadius: 8, border: 'none', cursor: isAlreadyRival(u.id) ? 'default' : 'pointer', background: isAlreadyRival(u.id) ? 'rgba(74,222,128,0.1)' : 'linear-gradient(135deg, #CC8800, #FFB800)', color: isAlreadyRival(u.id) ? '#4ade80' : '#000', fontFamily: 'Cinzel, serif', fontSize: 9, fontWeight: 700, letterSpacing: 1 }}
                >
                  {isAlreadyRival(u.id) ? '✓ RIVAL' : '+ RIVAL'}
                </button>
              </div>
